...
import { ViewportClampService } from "./ViewportClampService";
...
export class LabelOverlapToggleService {
  public static computeOverlapArea(a: OverlapRect, b: OverlapRect): number {
    const width = Math.max(0, Math.min(a.right, b.right) - Math.max(a.left, b.left));
    const height = Math.max(0, Math.min(a.bottom, b.bottom) - Math.max(a.top, b.top));
    return width * height;
  }

  public static doesLabelOverlapSpotlight(
    labelRect: OverlapRect,
    spotlightRect: OverlapRect,
    thresholdPx2 = LABEL_OVERLAP_AREA_THRESHOLD_PX2,
  ): boolean {
    return LabelOverlapToggleService.computeOverlapArea(labelRect, spotlightRect) > thresholdPx2;
  }
   ...
   */
  public static computeToggleButtonPosition(
    input: ToggleButtonPositionInput,
  ): { x: number; y: number } {
    const offsetPx = input.offsetPx ?? LabelOverlapToggleService.DEFAULT_TOGGLE_BUTTON_OFFSET_PX;
    ...
    const candidates = LabelOverlapToggleService.buildToggleCandidates(input, offsetPx, buttonSize / 2);
    return LabelOverlapToggleService.pickBestToggleAnchor(
      candidates,
      avoidRects,
      buttonSize,
      input.viewport,
   ...
   */
  public static computeLabelHideOffsetPx(
  ...
  private static readonly DEFAULT_TOGGLE_BUTTON_OFFSET_PX = 30;

  /** Minimum gap (px) kept between the button and the viewport edge. */
  private static readonly VIEWPORT_CLAMP_MARGIN_PX = 8;

  private static rectFromCenter(x: number, y: number, size: number): OverlapRect {
    const half = size / 2;
    return { top: y - half, right: x + half, bottom: y + half, left: x - half };
  }

  private static centerOf(rect: OverlapRect): { x: number; y: number } {
  ...
  private static buildToggleCandidates(
    ...
    const cornerAnchor = (corner: CornerName): { x: number; y: number } => {
      switch (corner) {
        case "bottom-right":
          return {
            x: input.viewport.width - LabelOverlapToggleService.VIEWPORT_CLAMP_MARGIN_PX - half,
            y: input.viewport.height - LabelOverlapToggleService.VIEWPORT_CLAMP_MARGIN_PX - half,
          };
        case "bottom-left":
          return {
            x: LabelOverlapToggleService.VIEWPORT_CLAMP_MARGIN_PX + half,
            y: input.viewport.height - LabelOverlapToggleService.VIEWPORT_CLAMP_MARGIN_PX - half,
          };
        case "top-left":
          return {
            x: LabelOverlapToggleService.VIEWPORT_CLAMP_MARGIN_PX + half,
            y: LabelOverlapToggleService.VIEWPORT_CLAMP_MARGIN_PX + half,
          };
        case "top-right":
          return {
            x: input.viewport.width - LabelOverlapToggleService.VIEWPORT_CLAMP_MARGIN_PX - half,
  ...
  private static pickBestToggleAnchor(
    ...
    const evaluate = (anchor: { x: number; y: number }) => {
      const rect = ViewportClampService.clampBoxToViewport(
        LabelOverlapToggleService.rectFromCenter(anchor.x, anchor.y, buttonSize),
        viewport,
        LabelOverlapToggleService.VIEWPORT_CLAMP_MARGIN_PX,
      );
      const score = avoidRects.reduce(
        (sum, avoidRect) => sum + LabelOverlapToggleService.computeOverlapArea(rect, avoidRect),
        0,
      );
      return { position: LabelOverlapToggleService.centerOf(rect), score };
    };
